...
subroutine limiter_low_order(u)
 ...
 !call compute_primitive(nodes,w,nx,ny,mx,my)
  ...
  ! Update variables with limited states
    ...
    subroutine limiter_positivity_3(u)
            use parameters_dg_2d
            implicit none
            real(kind=8),dimension(1:nvar,1:nx,1:ny,1:mx,1:my)::u, u_lim
            real(kind=8),dimension(1:nvar,1:nx,1:ny,1:mx,1:my)::w
    ...
    !end do
      ...
      !     do i = 1, mx
      !       do j = 1,my
      !         u_left=u_left+u_lim(ivar,icell,jcell,i,j)*legendre(chsi_left,i-1)*legendre(y_quad(intnode),j-1)
      !         u_right=u_right+u_lim(1,icell,jcell,i,j)*legendre(chsi_right,i-1)*legendre(y_quad(intnode),j-1)
      !       end do
      !     end do
      !     if(u_left<1d-10.OR.u_right<1d-10)then
      !        u_lim(1,icell,jcell,2:mx,1)=0.0
      !        exit
      !    end if
      !   end do
      !  end do
    ...
    !       do i = 1, mx
    !         do j = 1,my
    !           u_left=u_left+u_lim(1,icell,jcell,i,j)*legendre(chsi_left,j-1)*legendre(x_quad(intnode),i-1)
    !           u_right=u_right+u_lim(1,icell,jcell,i,j)*legendre(chsi_right,j-1)*legendre(x_quad(intnode),i-1)
    !         end do
    !       end do
    !       if(u_left<1d-10.OR.u_right<1d-10)then
    !          u_lim(1,icell,jcell,1,2:my)=0.0
    !          exit
    !      end if
    !     end do
    !   end do
      ...
      do icell=1,nx
        do jcell=1,ny
         ...
         !call compute_primitive(nodes,w,nx,ny,mx,my)
          ...
          !    do intnode = 1,mx
          !      do jntnode = 1,my
          !        if (w(1,icell,jcell,intnode,jntnode)<1e-7) then
          !          w(1,icell,jcell,intnode,jntnode) = 1
                    !write(*,*) 'lim'
          !        end if

          !        if (w(4,icell,jcell,intnode,jntnode)<1e-7) then
          !          w(4,icell,jcell,intnode,jntnode) = 1e-5
          !        end if
          ...
          call get_modes_from_nodes(nodes, u_lim, nx, ny, mx, my)
          ! Update variables with limited states
          u = u_lim
        end subroutine limiter_positivity_3
...
subroutine limiter_rossmanith(u)
          ...
          call get_nodes_from_modes(w, w_nodes, nx, ny, mx, my)

          ! get averages
          do icell = 1,nx
            do jcell = 1,ny
              w_average(1:nvar,icell,jcell) = w(1:nvar,icell,jcell,1,1)
              u_average(1:nvar,icell,jcell) = u(1:nvar,icell,jcell,1,1)
            end do
          end do
          ...
          !do ivar = 1,nvar
            do icell = 1,nx
              do jcell = 1,ny
                ...
                !end do
                ! get maximum in the neighborhood
                ! print*,'hi'
                ! periodic bcs
                  if (icell == 1) then
                  ...
                  do indx = 1,4
                    ...
                    w_average_stencil = w_average_stencil + w_nodes(ivar,indices_i(indx),indices_j(indx),1,1)
                    if (max_temp(ivar) > maximum_neigh_value(ivar)) then
                      maximum_neigh_value(ivar) = max_temp(ivar)
                    end if
                    if (min_temp(ivar) < minimum_neigh_value(ivar)) then
                      minimum_neigh_value(ivar) = min_temp(ivar)
                    end if
                    ! check the corners too
                  end do

!                  maximum_neigh_value(ivar) = max(w_average(ivar,icell,jcell)+500*dx**1.5,maximum_neigh_value(ivar))
!                  minimum_neigh_value(ivar) = min(w_average(ivar,icell,jcell)-500*dx**1.5,minimum_neigh_value(ivar))
                  maximum_neigh_value(ivar) = max(w_average_stencil/dble(4),maximum_neigh_value(ivar))
                  minimum_neigh_value(ivar) = min(w_average_stencil/dble(4),minimum_neigh_value(ivar))


                  ! these theta are the same for all variables (why?)
                  ratio1(ivar) = (minimum_neigh_value(ivar)-w_average(ivar,icell,jcell))/&
                  &dble(minimum_cell_value(ivar)-w_average(ivar,icell,jcell))
                  ratio2(ivar) = (maximum_neigh_value(ivar)-w_average(ivar,icell,jcell))/&
                  &dble(maximum_cell_value(ivar)-w_average(ivar,icell,jcell))
                  !ratio1(ivar) = (minimum_neigh_value(ivar)-w_average_stencil/dble(4))/&
                  !&dble(minimum_cell_value(ivar)-w_average_stencil/dble(4))
                  !ratio2(ivar) = (maximum_neigh_value(ivar)-w_average_stencil/dble(4))/&
                  !&dble(maximum_cell_value(ivar)-w_average_stencil/dble(4))
                ...
                theta = min(dble(1),min(little_theta,big_theta))
                !print*,theta
                do ivar = 1,nvar
                  do intnode = 1,mx
                    do jntnode = 1,mx
                ...
                do intnode = 1,mx
                  do jntnode = 1,mx
                    if ((ivar==1).or.(ivar==4)) then
                      !print*,w_nodes(ivar,icell,jcell,intnode,jntnode)
                      w_nodes(ivar,icell,jcell,intnode,jntnode) = max(1e-6,w_nodes(ivar,icell,jcell,intnode,jntnode))
                    end if
                  end do
                end do
              end do
